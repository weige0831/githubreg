...
Windows 上**关掉远程桌面窗口后，Chrome 会把窗口判定成"被遮挡/不可见"**，然后按隐藏页面的规则处理：
定时器降频（隐藏超过 5 分钟且是链式定时器时降到每分钟一次），更久了还可能把标签页冻结
（JS 全停，等窗口重新可见才恢复）—— 表现就是"我一关远程桌面，它就卡住不动了"。

**插件侧已经做了三件事**（都在代码里，不用配置）：

1. **防降频保活**：内容脚本用两个本地 `RTCPeerConnection` 建一条环路（不出网、不产生流量）
   并保持 data channel 打开，同时握一个 Web Lock。Chrome 对「正在用 WebRTC」的页面**不降频**，
   所以只要保活成功，断开远程桌面后流程基本照常跑。日志里会有一行
   `🛡 防节流保活已开启`。
2. **常驻心跳**：页面每 15 秒给后台报一次（不算日志）。
3. **后台看门狗**（`chrome.alarms`，每分钟一次，**不受页面降频影响**），两级处理：
   - 60 秒没动静 → 温和地拉一把（激活标签页 + 叫它一声），**不导航、不丢进度**；
   - 3 分钟没动静 → 判定被冻结了，重新打开页面（按 `stage` 接着跑）。

**怎么判断你是哪种情况**（看面板日志）：

| 日志 | 含义 | 还能做点什么 |
| --- | --- | --- |
| `⏱️ 页面心跳间隔变成 N 秒…被降频了，但还在跑` | 保活没生效，页面被降频但没死 | 用下面的第 1 或第 2 条根治 |
| `⏱️ 页面 N 秒没动静：拉了一下标签页` | 冻结过一次，已被唤醒 | 同上 |
| `⏱️ 页面已经 N 分钟没有任何动静…重新打开页面` | 页面被彻底冻结，只能重开 | 必须用下面的办法根治 |
| `🛡 防节流保活已开启` 之后没有上面几条 | 保活起效，断开远程桌面也能继续 | 不用管 |

**根治（二选一，插件没有 API 能替代）**：

1. **带参数启动 Chrome**（桌面上的 `Chrome-防冻结启动.bat` 就是这个）：
...
3. 另外建议在 Chrome 设置 → 性能里**关掉「内存节省程序」**（否则不活跃的标签页可能被冻结/回收；
   扩展侧已经给任务标签页加了 `autoDiscardable: false`，两边都关掉最稳）。
