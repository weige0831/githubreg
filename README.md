...
1. **页面出现 GitHub 限流**（`too many requests` / `rate limit` / `whoa there` / `try again later`）——
   拉黑当前节点、换一个、**刷新当前页面**接着跑；同一个号最多连换 3 次，避免一直刷。
2. **节点测不通或延迟超过阈值**——批量进行中每分钟检查一次（`chrome.alarms`），换掉并刷新页面；
   另外**每开一个新号之前**也会检查一次，节点不行就先换，免得新号也跑不动。

> 选节点时会测 3 个候选取最快的；被换掉的节点进黑名单（时长可配）。
> **分组要用「手动选择 / Selector」类型**——`url-test`、`fallback` 这类分组 Clash 自己会改回去，拉黑就不生效了。
> 这个功能默认关闭，需要时在面板里打开并保存（会申请控制器地址的访问权限）。

> **Clash Verge 在哪找地址和密钥**：设置 → 「Clash 设置 / 端口设置」→ 外部控制（External Controller）的地址与密钥，密钥旁边有显示/复制按钮。
> 地址填错或密钥不对时，点「测试连接」会**自动探测常见端口并给出结论**：在哪个端口找到了控制器、是缺密钥还是密钥写错了（探测到的地址会自动填进输入框）。
