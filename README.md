...
2. **遇到「访问暂时受限」（人机验证拦截）或页面卡住**——**不走换节点那套**，而是重置会话从头跑：
   - 第 1~3 次：清空 GitHub 的 cookie + localStorage（只清 github.com，不动其它站点登录态），
     然后从 GitHub 首页重新开始这个号的注册；
   - 第 4~7 次：连节点一起换（拉黑当前节点）+ 等 30 秒再重开——清会话解决不了 IP 层面的判定，
     所以到这时候两种办法一起上；
   - 超过 7 次：这个号**直接丢弃**（不写账户列表、不进管理器，避免留一堆没 token 的空号），
     只把邮箱/用户名/密码打进日志备查，然后继续下一个，免得整批卡在一个号上；
   - 如果发生在 token 阶段（已登录），清 cookie 等于登出，会直接换一个新邮箱重开这个位置。
