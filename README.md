...
1. **页面出现 GitHub 限流**（`too many requests` / `rate limit` / `whoa there` / `try again later`）——
   按下面的循环自动处理，直到不再出现限流为止：
   1. **换一个节点**（按分组顺序换下一个，此时不拉黑），页面重新打开；
   2. 在**同一个节点上连续刷新最多 10 次**（每次页面加载算一次，页面内不会重复计数）；
   3. 刷满 10 次还限流 → **等 1 分钟**，在同一节点上再刷 10 次；
   4. 还是限流 → **把当前节点拉黑 10 分钟，换下一个节点**，回到第 2 步继续；
...
> 被拉黑的节点进黑名单（时长可配，默认 10 分钟），期间不会被选中。
